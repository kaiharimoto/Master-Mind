#!/usr/bin/env node
// Builds the three critic briefs for a cycle.
//
// §04/§06: each critic is a fresh-context session and receives ONLY the mission
// goal, the rubric, the current evidence set, the previous cycle's evidence set
// and the DIRECTION.md decision record. Nothing else — in particular no code,
// no object tree, no MANIFEST, and none of the builder's own summary.
...
demo audience would: what the frames show, nothing promised in prose.`,
  ...
  auditor: `## Your categories — both are HARD GATES that block exit at any score
...
export function build(cycle, curDir, prevDir) {
  const direction = readFileSync(resolve(ROOT, 'DIRECTION.md'), 'utf8');
  const outDir = resolve(ROOT, 'evidence/critic-briefs');
  mkdirSync(outDir, { recursive: true });
  const list = (dir) => ARTIFACTS.map(([id, file, desc]) => {
    const p = join(dir, file);
    const sheet = join(dir, 'sheets', file.replace('.mp4', '_sheet.png'));
    const have = existsSync(p);
    const line = `- **${id}** \`${file}\` — ${desc}\n  - file: ${have ? p : 'NOT PRESENT'}`;
    return file.endsWith('.mp4') && existsSync(sheet)
      ? `${line}\n  - frames (timestamped contact sheet, 20 frames across the take): ${sheet}`
      : line;
  }).join('\n');

  const hasPrev = existsSync(join(prevDir, ARTIFACTS[0][1]));
  const written = [];
  for (const [name, rubric] of Object.entries(RUBRIC)) {
    const body = `${GOAL}
...
across the take. Read \`${resolve(ROOT, 'DIRECTION.md')}\` for the decision
record — it tells you what the builder committed to, so you can check whether
the frames honour it.

${hasPrev ? `## The previous cycle's evidence set

The previous cycle's artifacts are in \`${prevDir}\` under the same filenames.
...
There is no previous cycle — this is cycle ${cycle}. Label every artifact **new**.`}

# Current evidence set — cycle ${cycle}
...
have moved between cycles without an explicit user act.` : ''}${name === 'art-director' ? `
Additionally include, at the top level:
...
`;
    const f = join(outDir, `${name}-cycle-${cycle}.md`);
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  const cur = resolve(ROOT, process.argv[3] ?? 'evidence');
  const prev = resolve(ROOT, process.argv[4] ?? `evidence/history/cycle-${cycle - 1}`);
  for (const f of build(cycle, cur, prev)) console.log('  ' + f);
}
